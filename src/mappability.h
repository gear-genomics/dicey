...
namespace dicey
{
  ...
  int mappability(int argc, char** argv) {
    ...
    for(uint32_t refIndex = 0; refIndex < nchr; ++refIndex) {
      ...
      of << ">" << seqname << std::endl;
      if (seqlen < c.readlength) {
	for(int32_t pos = 0; pos < seqlen; ++pos) of << 'N';
	of << std::endl;
      } else {
	for(int32_t pos = 0; pos < halfwin; ++pos) of << 'N';
	for(int32_t pos = 0; pos < (seqlen - c.readlength + 1); ++pos) {
	  ...
	  if (!nContent) {
	    ...
	    if (maxED == -1) of << 'N';
	    else if (maxED == 0) of << 'A';
	    else if (maxED == 1) of << 'C';
	    else if (maxED == 2) of << 'G';
	    else of << 'T';
	  } else {
	    of<< 'N';
	  }
